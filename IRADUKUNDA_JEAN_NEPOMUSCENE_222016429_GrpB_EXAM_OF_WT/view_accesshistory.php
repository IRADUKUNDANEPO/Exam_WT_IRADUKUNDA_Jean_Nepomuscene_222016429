...
<head>
  ...
  <title> Retrieve History of Access the Documents</title>
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail Information of Access History</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Access History</h2></center>
    <table border="5">
        <tr>
            <th>Access History ID</th>
            <th>Document ID</th>
            <th>User ID</th>
            <th>Access Date</th>
            <th>Delete</th>
        ...
        // Prepare SQL query to retrieve all access history records
        $sql = "SELECT * FROM AccessHistory";
        $result = $connection->query($sql);

        // Check if there are any access history records
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $accessHistoryId = $row['AccessHistoryID']; // Fetch the AccessHistoryID
                echo "<tr>
                    <td>" . $row['AccessHistoryID'] . "</td>
                    <td>" . $row['DocumentID'] . "</td>
                    <td>" . $row['UserID'] . "</td>
                    <td>" . $row['AccessDate'] . "</td>
                    <td><a style='padding:4px' href='delete_accesshistory.php?AccessHistoryID=$accessHistoryId'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_accesshistory.php?AccessHistoryID=$accessHistoryId'>Update</a></td> 
                </tr>";
            }
        } else {
